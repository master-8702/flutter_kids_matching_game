...
/// This widget is used to show the fruit game screen, where the user can
/// drag the left side names of fruits and drop them on right side of the fruit
/// pictures.
class FruitGameScreen extends ConsumerStatefulWidget {
  const FruitGameScreen({Key? key}) : super(key: key);
  @override
  FruitGameScreenState createState() => FruitGameScreenState();
}

class FruitGameScreenState extends ConsumerState<FruitGameScreen> {
  ...
  void _restartGame() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref
          ...
          .initGame(currentLevel: level.number, gameType: GameType.fruit);
    });
  }
  ...
  Widget build(BuildContext context) {
    final fruitGameController = ref.watch(gameScreenNotifierProvider);
    final isGameOver = fruitGameController.isGameOver;
    return Container(
      color: AppTheme.primaryColor.withAlpha(0),
      child: Scaffold(
        // backgroundColor: AppTheme.primaryColor.withAlpha(25),
        appBar: AppBar(
          title: Text(
            '${'score'.tr()} :  ${fruitGameController.score}',
